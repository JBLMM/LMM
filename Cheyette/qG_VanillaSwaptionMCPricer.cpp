...
qG_VanillaSwaptionMCPricer::qG_VanillaSwaptionMCPricer(	CheyetteDD_Model_PTR cheyetteModel_PTR,
														VanillaSwaption vanillaSwaption,
														RNGenerator_PTR rnGenerator)
					: cheyetteModel_PTR_(cheyetteModel_PTR), vanillaSwaption_(vanillaSwaption), rnGenerator_(rnGenerator){}
...
double qG_VanillaSwaptionMCPricer::TrajectoireEuler(int nbPas) const
{
	std::vector<double>   gaussian_tmp(1,0.); 
	rnGenerator_->generate(gaussian_tmp);  // generate Gaussian.

	double T0 = vanillaSwaption_.getUnderlyingSwap().get_StartDate() ;  //date d'entrée dans le swap
...
double qG_VanillaSwaptionMCPricer::price(const VanillaSwaption& vanillaSwaption, size_t nbSimulation, double nbPas)  const
{
	double result = 0.0;
	//double variance = 0.0;
